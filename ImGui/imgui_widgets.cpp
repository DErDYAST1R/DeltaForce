...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    // opening anim.
    clpick_anim = ImLerp(clpick_anim, (clpick_anim < 1.0f ? 1.0f : 0.0f), 0.20f * (1.0f - ImGui::GetIO().DeltaTime));
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(9, 9));
        ...
        if (ColorButton(label, col_v4, flags, ImVec2(15, 15)))
        {
        ...
        if (BeginPopup("picker", ImGuiWindowFlags_NoMove))
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                ...
                SetNextItemWidth(clpick_anim * square_sz * 8.0f); // Use 256 + bar sizes?
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    {
        // Render SV Square
        draw_list->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), ImColor(225, 226, 230), col_white, hue_color32, hue_color32, col_white, 8.0f);
        draw_list->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), ImColor(225, 226, 230), 0, 0, col_black, col_black, 8.0f);
        RenderFrameBorder(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), 8.0f);
        ...
        // Render Hue Bar
        draw_list->AddRectFilledMultiColorRounded(ImVec2(bar0_pos_x, picker_pos.y + 0 * (sv_picker_size / 6)), ImVec2(bar0_pos_x + bars_width, picker_pos.y + (0 + 1) * (sv_picker_size / 6)), ImColor(225, 226, 230), col_hues[0], col_hues[0], col_hues[0 + 1], col_hues[0 + 1], 6.0f, ImDrawFlags_RoundCornersTop);

        for (int i = 1; i < 5; ++i)
            draw_list->AddRectFilledMultiColor(ImVec2(bar0_pos_x, picker_pos.y + i * (sv_picker_size / 6)), ImVec2(bar0_pos_x + bars_width, picker_pos.y + (i + 1) * (sv_picker_size / 6)), col_hues[i], col_hues[i], col_hues[i + 1], col_hues[i + 1]);

        draw_list->AddRectFilledMultiColorRounded(ImVec2(bar0_pos_x, picker_pos.y + 5 * (sv_picker_size / 6)), ImVec2(bar0_pos_x + bars_width, picker_pos.y + (5 + 1) * (sv_picker_size / 6)), ImColor(225, 226, 2303), col_hues[5], col_hues[5], col_hues[5 + 1], col_hues[5 + 1], 6.0f, ImDrawFlags_RoundCornersBottom);

        //for (int i = 0; i < 6; ++i)
        //    draw_list->AddRectFilledMultiColorRounded(ImVec2(bar0_pos_x, picker_pos.y + i * (sv_picker_size / 6)), ImVec2(bar0_pos_x + bars_width, picker_pos.y + (i + 1) * (sv_picker_size / 6)), ImColor(33, 33, 33), col_hues[i], col_hues[i], col_hues[i + 1], col_hues[i + 1], 5.0f);
        float bar0_line_y = IM_ROUND(picker_pos.y + H * sv_picker_size);
        RenderFrameBorder(ImVec2(bar0_pos_x, picker_pos.y), ImVec2(bar0_pos_x + bars_width, picker_pos.y + sv_picker_size), 6.0f);
        RenderArrowsForVerticalBar(draw_list, ImVec2(bar0_pos_x - 1, bar0_line_y), ImVec2(bars_triangles_half_sz + 1, bars_triangles_half_sz), bars_width + 2.0f, style.Alpha);
    }

    // Render cursor/preview circle (clamp S/V within 0..1 range because floating points colors may lead HSV values to be out of range)
    float sv_cursor_rad = value_changed_sv ? 10.0f : 6.0f;
    draw_list->AddCircle(sv_cursor_pos + ImVec2(0, 1), 4.0f, ImColor(0.0f, 0.0f, 0.0f, 0.6f), 30);
    draw_list->AddCircle(sv_cursor_pos, 4.0f, col_white, 30);
    ...
    if (alpha_bar)
    {
        ...
        draw_list->AddRectFilledMultiColorRounded(bar1_bb.Min, bar1_bb.Max, ImColor(225, 226, 230), user_col32_striped_of_alpha, user_col32_striped_of_alpha, user_col32_striped_of_alpha & ~IM_COL32_A_MASK, user_col32_striped_of_alpha & ~IM_COL32_A_MASK, 6.0f);
        float bar1_line_y = IM_ROUND(picker_pos.y + (1.0f - alpha) * sv_picker_size);
        RenderFrameBorder(bar1_bb.Min, bar1_bb.Max, 6.0f);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    ItemSize(frame_bb, (size.y >= default_size) ? g.Style.FramePadding.y : 0.0f);
    if (!ItemAdd(bb, id))
        return false;
    ...
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
        if (g.Style.FrameBorderSize > 0.0f)
            window->DrawList->AddRect(bb.Min, bb.Max, ImColor(0.0f, 0.0f, 0.0f, 0.10f), rounding); // Color button are often in need of some sort of border
        else
            window->DrawList->AddRect(bb.Min, bb.Max, ImColor(0.0f, 0.0f, 0.0f, 0.10f), rounding); // Color button are often in need of some sort of border
    }
