...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    float anim = ImTricks::Animations::FastFloatLerp( label, 0, hovered, 0.f, 1.f, ui::anim_speed );
    auto outline_col = ImTricks::Animations::FastColorLerp( ImGui::GetColorU32( ImGuiCol_Border ), ImGui::GetColorU32( ImGuiCol_Scheme, 0.5f ), anim );

    // Render
    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderFrame(bb.Min, bb.Max, col, 0, 3);
    window->DrawList->AddRect( bb.Min, bb.Max, outline_col, 3 );
    RenderTextClipped( bb.Min, bb.Max, label, FindRenderedTextEnd( label ), 0, { 0.5f, 0.5f } );
...
bool ImGui::Checkbox( const char* label, bool *v ) {
    ...
    float square_sz = 21.f;
    ImRect frame_bb( window->DC.CursorPos, window->DC.CursorPos + ImVec2( square_sz, square_sz ) );
    ImRect total_bb( window->DC.CursorPos, window->DC.CursorPos + ImVec2( square_sz + label_size.x + 7, square_sz ) );
    ...
    float anim = ImTricks::Animations::FastFloatLerp( label, 0, *v, 0.f, 1.f, ui::anim_speed );
    float frame_col_anim = ImTricks::Animations::FastFloatLerp( label, 1, hovered, 0.f, 1.f, ui::anim_speed );

    auto col = ImTricks::Animations::FastColorLerp( GetColorU32( ImGuiCol_FrameBg ), GetColorU32( ImGuiCol_FrameBgHovered ), frame_col_anim );

    window->DrawList->AddRectFilled( frame_bb.Min, frame_bb.Max, col, 3 );
    window->DrawList->AddRect( frame_bb.Min, frame_bb.Max, GetColorU32( ImGuiCol_Border ), 3 );
    glow::rect_glow( frame_bb.Min, frame_bb.Max, GetColorU32( ImGuiCol_Scheme, anim * 0.07f ), 10, 4 );
    window->DrawList->AddRectFilled( frame_bb.Min, frame_bb.Max, GetColorU32( ImGuiCol_Scheme, anim ), 3 );
    window->DrawList->AddText( { total_bb.Max.x - label_size.x, frame_bb.GetCenter( ).y - label_size.y / 2 }, GetColorU32( ImGuiCol_Text ), label, FindRenderedTextEnd( label ) );

    RenderCheckMark( window->DrawList, frame_bb.GetCenter( ) - ImVec2( ( square_sz / 2 + 1 ) / 2, ( square_sz / 2 + 1 ) / 2 ), GetColorU32( ImGuiCol_CheckMark, anim ), square_sz / 2 + 1 );
...
bool ImGui::Hotkey( const char* label, int* k ) 
{
    ...
    float w = GetWindowWidth( ) - GetStyle( ).WindowPadding.x * 2;
    ImVec2 size{ CalcTextSize( KeyNames[*k] ).x + 20, 28 };
    float width = ImTricks::Animations::FastFloatLerp( label, 0, 1, 0.f, size.x, ui::anim_speed );
    ImRect bb( window->DC.CursorPos + ImVec2{ w - width, 0 }, window->DC.CursorPos + ImVec2{ w, size.y } );
    ImRect total_bb( window->DC.CursorPos, window->DC.CursorPos + ImVec2{ w, size.y } );
    ItemAdd( total_bb, id );
    ItemSize( total_bb );
    ...
    float anim = ImTricks::Animations::FastFloatLerp( label, 1, hovered || g.ActiveId == id, 0.f, 1.f, ui::anim_speed );
    auto col = ImTricks::Animations::FastColorLerp( ImGui::GetColorU32( ImGuiCol_Border ), ImGui::GetColorU32( ImGuiCol_Scheme, 0.5f ), anim );

    window->DrawList->AddText( { total_bb.Min.x, bb.GetCenter( ).y - CalcTextSize( label, 0, 1 ).y / 2 }, GetColorU32( ImGuiCol_Text ), label, FindRenderedTextEnd( label ) );
    window->DrawList->AddRectFilled( bb.Min, bb.Max, GetColorU32( hovered || g.ActiveId == id ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg ), 3 );
    window->DrawList->AddRect( bb.Min, bb.Max, col, 3 );
    RenderTextClipped( bb.Min, bb.Max, buf_display.c_str( ), 0, 0, { 0.5f, 0.5f } );
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    ImRect bb( window->DC.CursorPos + ImVec2( window->Size.x - style.WindowPadding.x * 2 - 120, 0 ), window->DC.CursorPos + ImVec2( window->Size.x - style.WindowPadding.x * 2, 28 ) );
    ImRect total_bb( window->DC.CursorPos, bb.Max );
    ...
    float anim = ImTricks::Animations::FastFloatLerp( label, 0, hovered || popup_open, 0.f, 1.f, ui::anim_speed );
    float arrow_anim = ImTricks::Animations::FastFloatLerp( label, 1, popup_open, 0.f, 1.f, ui::anim_speed );
    auto col = ImTricks::Animations::FastColorLerp( GetColorU32( ImGuiCol_FrameBg ), GetColorU32( ImGuiCol_FrameBgHovered ), anim );
    auto outline_col = ImTricks::Animations::FastColorLerp( GetColorU32( ImGuiCol_Border ), GetColorU32( ImGuiCol_Scheme, 0.5f ), anim );
    auto arrow_col = ImTricks::Animations::FastColorLerp( GetColorU32( ImGuiCol_CheckMark, 0.25f ), GetColorU32( ImGuiCol_Scheme ), anim );

    RenderFrame( bb.Min, bb.Max, col, 0, 3 );
    window->DrawList->AddRect( bb.Min, bb.Max, outline_col, 3 );
    RenderText( { total_bb.Min.x, bb.GetCenter( ).y - label_size.y / 2 }, label );

    PushStyleColor( ImGuiCol_Text, ( hovered || popup_open ? GetColorU32( ImGuiCol_Text ) : GetColorU32( ImGuiCol_TextDisabled ) ) );
    RenderTextClipped( ImVec2( bb.Min.x + 5, bb.GetCenter( ).y - label_size.y / 2 ), ImVec2( bb.Max.x - 25, bb.GetCenter( ).y + label_size.y / 2 + 1 ), preview_value, NULL, NULL, ImVec2( 0.f, 0.f ) );
    PopStyleColor( );

    window->DrawList->AddLine( { bb.Max.x - 23, bb.Min.y + 8 }, { bb.Max.x - 23, bb.Max.y - 8 }, outline_col );

    glow::rect_glow( { bb.Max.x - 14, bb.GetCenter( ).y - 2 * anim }, { bb.Max.x - 10, bb.GetCenter( ).y + 2 * anim }, ImColor( arrow_col.Value.x, arrow_col.Value.y, arrow_col.Value.z, ( arrow_col.Value.w * 0.08f ) * anim ), 10, 4 * anim );

    ui::ImRotateStart( );
    RenderArrow( window->DrawList, ImVec2( bb.Max.x - 14, bb.GetCenter( ).y - 10.f ), arrow_col, ImGuiDir_Right, 13.f );
    ui::ImRotateEnd( IM_PI * arrow_anim );
    ...
    else
    {
        ...
        SetNextWindowSizeConstraints(ImVec2(w, 0.0f), ImVec2(FLT_MAX, CalcMaxPopupHeightFromItemCount(popup_max_height_in_items) * anim));
    }
    ...
    PushStyleVar( ImGuiStyleVar_WindowPadding, ImVec2( 3, 4 ) ); // Horizontally align ourselves with the framed text
    PushStyleVar( ImGuiStyleVar_PopupRounding, 3 );
    PushStyleVar( ImGuiStyleVar_PopupBorderSize, 1 );
    PushStyleColor( ImGuiCol_PopupBg, col.Value );
    PushStyleColor( ImGuiCol_Border, outline_col.Value );
...
bool ImGui::InputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    ...
    return InputTextEx(label, NULL, buf, (int)buf_size, ImVec2(120, 28), flags, callback, user_data);
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2{ window->Size.x - style.WindowPadding.x * 2 - frame_size.x, 0 }, window->DC.CursorPos + ImVec2{ window->Size.x - style.WindowPadding.x * 2, frame_size.y });
    ...
    float anim = ImTricks::Animations::FastFloatLerp( label, 0, hovered || IsItemActive( ), 0.f, 1.f, ui::anim_speed );
    auto col = ImTricks::Animations::FastColorLerp( ImGui::GetColorU32( ImGuiCol_Border ), ImGui::GetColorU32( ImGuiCol_Scheme, 0.5f ), anim );

    // Render frame
    if (!is_multiline) {
        RenderFrame( frame_bb.Min, frame_bb.Max, GetColorU32( ImGuiCol_FrameBg ), 0, 3 );
        window->DrawList->AddRect( frame_bb.Min, frame_bb.Max, col, 3 );
    }
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        PushStyleVar( ImGuiStyleVar_ItemInnerSpacing, ImVec2( 8, 10 ) );
        PushStyleColor( ImGuiCol_PopupBg, ImVec4( ImColor( 25, 28, 37 ) ) );
        ...
        if (BeginPopup("picker"))
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                ...
                SetNextItemWidth(square_sz * 7.3f); // Use 256 + bar sizes?
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    {
        // Render SV Square
        draw_list->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), GetColorU32( ImGuiCol_PopupBg ), col_white, hue_color32, hue_color32, col_white, 3);
        draw_list->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), GetColorU32( ImGuiCol_PopupBg ), 0, 0, col_black, col_black, 3);
        ...
        ui::render_circle_for_horizontal_bar( ImVec2( bar0_line_x + 1, picker_pos.y + sv_picker_size + style.ItemInnerSpacing.y + 2 ), hue_color32, style.Alpha );
    }

    // Render cursor/preview circle (clamp S/V within 0..1 range because floating points colors may lead HSV values to be out of range)
    float sv_cursor_rad = value_changed_sv ? 3.0f : 4.0f;
    ...
    if (alpha_bar)
    {
        ...
        draw_list->AddRectFilledMultiColorRounded(bar1_bb.Min, bar1_bb.Max, GetColorU32( ImGuiCol_PopupBg ), ImColor( col[0], col[1], col[2], style.Alpha ), ImColor( col[0], col[1], col[2], 0.f * style.Alpha ), ImColor( col[0], col[1], col[2], 0.f * style.Alpha ), ImColor( col[0], col[1], col[2], style.Alpha ), 3 );
        float bar1_line_x = IM_ROUND(picker_pos.x + (1.0f - alpha) * sv_picker_size);
        ui::render_circle_for_horizontal_bar( ImVec2( bar1_line_x + 1, bar1_bb.GetCenter( ).y ), ImColor( col[0], col[1], col[2] ), style.Alpha );
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    const ImVec2 text_min = pos + ImVec2{ 2, 0 };
    ...
    // Render
    RenderFrame( bb.Min, bb.Max, ImGui::GetColorU32( ImGuiCol_Text, 0.02f * anim ), 0 );
